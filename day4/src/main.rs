...
fn star_one(input_str: &str) -> u32 {
    ...
    for input_line in input_str.lines() {
        ...
        let num_of_matching_tickets = game_data.my_tickets.intersection(&game_data.winning_tickets).cloned().collect::<Vec<u32>>().len();
...
fn star_two(input_str: &str) -> u32 {
    ...
    for (id, num_wins) in num_wins_per_id_vec {
        ...
        for i in 1..num_wins+1 {
            *copies.entry(id+i as u32).and_modify(|v| *v += (1*number_of_copies_for_this_id)).or_insert(1);
        }
    }
